...
namespace ImageManager
{
    ...
    /* Constructor */
    Image::Image(int width, int height) : width(width), height(height), pixel(new unsigned char[width * height * 3])
    {
        int i;
    ...
    Image &Image::operator=(const Image &other_image)
    {
        ...
        // Free current object to avoid memory leak
        delete[] pixel;
    ...
    Image::Image(Image &&other_image) noexcept
    {
        ...
        width = other_image.width;
        height = other_image.height;
        pixel = other_image.pixel;

        other_image.width = 0;
        other_image.height = 0;
        other_image.pixel = nullptr;
    }
